...
/* macros */
#define CMDLINEOPTS	"Ab:c:d:De:fF:Ggk:lL:no:Op:P:Qrs:S:t:T:u:vVWx:X?"
...
static void dkimf_log(struct dkimf_config *, int, const char *, ...);
...
dkimf_xs_signfor(lua_State *l)
{
	...
	status = dkim_mail_parse((u_char *) addr, &user, &domain);
...
dkimf_xs_parsefield(lua_State *l)
{
	...
	strlcpy((char *) field, lua_tostring(l, 1), sizeof field);
	lua_pop(l, 1);
...
dkimf_xs_requestsig(lua_State *l)
{
	...
	if (keyname != NULL)
	{
		                              ...
		                              signlen))
		{
		  ...
		  case 2:
			dkimf_log(conf, LOG_ERR, "key '%s' could not be loaded",
			       keyname);
			lua_pushnumber(l, 0);
			return 1;
		  ...
		  case -1:
			dkimf_log(conf, LOG_ERR, "error requesting key '%s'",
			       keyname);
			lua_pushnumber(l, 0);
			return 1;
		}
...
dkimf_xs_getenvfrom(lua_State *l)
{
	...
	if (ctx == NULL)
		...
		lua_pushstring(l, (char *) dfc->mctx_envfrom);
	return 1;
}
...
dkimf_xs_verify(lua_State *l)
{
	...
	if (ctx != NULL)
	{
		...
		if (dfc->mctx_dkimv == NULL)
		{
			...
			dfc->mctx_dkimv = dkim_verify(conf->conf_libphoenixdkim,
			                              dfc->mctx_jobid, NULL,
			                              &status);
...
dkimf_xs_delrcpt(lua_State *l)
{
	...
	{
		dkimf_log(conf, LOG_ERR, "%s: smfi_delrcpt() failed",
		       dfc->mctx_jobid);
	}
	else
	{
		...
		{
			dkimf_log(conf, LOG_ERR, "%s: smfi_addheader() failed",
			       dfc->mctx_jobid);
		}
	}
...
/*
**  DKIMF_LOG -- log messages via syslog(3) and/or to standard output (stdout)
**
**  Parameters:
...
static void
dkimf_log(struct dkimf_config *conf, int priority, const char *format, ...)
{
	va_list args;
	va_list tmp;
	const char *prefix;
		...
		*/
		    ...
		    dbd[3].dbdata_buflen != (size_t) -1)
		{
			...
			if (declared == -1)
			{
				if (dolog)
				{
					dkimf_log(curconf, strict ? LOG_ERR : LOG_WARNING,
...
dkimf_reportaddr(struct dkimf_config *conf)
{
	...
	if (pw == NULL)
	{
		...
		{
			if (dolog)
				dkimf_log(curconf, LOG_ERR, "reportaddr truncated");
			return;
		}
	}
	else
...

	{
		...
		{
			if (dolog)
				dkimf_log(curconf, LOG_ERR, "reportcmd truncated");
			return;
		}
	}
}
...
dkimf_config_free(struct dkimf_config *conf)
{
	...
	if (conf->conf_libphoenixdkim != NULL)
		dkim_close(conf->conf_libphoenixdkim);
                  ...
                  char *err, size_t errlen, char *become)
{
	...
	if (data != NULL)
	{
		...
		if (!conf->conf_dolog_syslog)
		{
			(void) config_get(data, "Syslog",
			                  &conf->conf_dolog_syslog,
			                  sizeof conf->conf_dolog_syslog);
		}

		if (!conf->conf_dolog_stdout)
		{
			(void) config_get(data, "StdoutLog",
			                  &conf->conf_dolog_stdout,
...
dkimf_config_setlib(struct dkimf_config *conf, const char **err)
{
	...
	*/
	...
	if (conf->conf_sendreports || conf->conf_keeptmpfiles ||
	    ...
	    conf->conf_fixcrlf)
	{
		status = dkim_getopt(conf->conf_libphoenixdkim,
		...
		status = dkim_setopt(conf->conf_libphoenixdkim,
		                      DKIM_OPTS_FLAGS, &opts, sizeof opts);
	...
	status = dkim_set_prescreen(conf->conf_libphoenixdkim, dkimf_prescreen);
	if (status != DKIM_STAT_OK)
	{
		if (err != NULL)
...
dkimf_checkbldb(DKIMF_DB db, char *to, const char *jobid)
{
	...
	status = dkim_mail_parse((u_char *) addr, (u_char **) &user,
	                         (u_char **) &domain);
	if (status != 0 || user == NULL || domain == NULL)
	{
		if (dolog)
...
dkimf_sendprogress(const void *ctx)
{
	if (ctx != NULL)
	{
		...
		/*
		**  The dns_callback signature delivers ctx as const void *;
		**  the value stored there is a live SMFICTX * that smfi
		**  entry points mutate -- cast away const to recover the
...
dkimf_log_ssl_errors(DKIM *dkim, DKIM_SIGINFO *sig, const char *jobid)
{
	...
	if (sig != NULL)
	{
		domain = (char *) dkim_sig_getdomain(sig);
		selector = (char *) dkim_sig_getselector(sig);
	...
	if (errbuf != NULL)
	{
		...
		{
			dkimf_log(curconf, LOG_INFO, "%s: s=%s d=%s SSL %s",
			          jobid, selector, domain, errbuf);
		}
		else
		{
	...
	switch (status)
	{
	  ...
	  case DKIM_STAT_NOKEY:
		...
		if (conf->conf_dolog)
		{
			...
			{
				dkimf_log(conf, LOG_ERR, "%s: %s%s%s",
	...
	if (dstatus == DKIM_STAT_OK)
	{
		...
		for (c = 0; c < nsigs; c++)
		{
			...
			*/
			...
			algorithm = (char *) dkim_sig_getalgorithm(sigs[c]);
...
mlfi_connect(SMFICTX *ctx, char *host, _SOCK_ADDR *ip)
{
	...
	if (!dkimf_dns_init(conf->conf_libphoenixdkim, conf, &err))
	{
		dkimf_log(conf, LOG_ERR, "can't initialize resolver: %s", err);
	...
	if (cc == NULL)
	{
		...
		if (cc == NULL)
		{
			dkimf_log(conf, LOG_ERR, "%s malloc(): %s", host,
			       strerror(errno));
...
mlfi_header(SMFICTX *ctx, char *headerf, char *headerv)
{
	...
	*/
	...
	{
		dkimf_log(conf, LOG_NOTICE, "ignoring header field '%s'",
		       headerf);
...
mlfi_eoh(SMFICTX *ctx)
{
	...
	*/
	...
	if (conf->conf_sendermacro != NULL)
	{
		...
		if (macrosender != NULL)
			dkimf_dstring_copy(addr, (u_char *) macrosender);
	}
  	...
  	if (dkimf_dstring_len(addr) == 0)
	{
  	...
  	if (from != NULL)
		dkimf_dstring_copy(addr, (u_char *) from->hdr_val);
	...
	*/
	...
	if (dfc->mctx_srhead != NULL)
	{
		...
		for (sr = dfc->mctx_srhead; sr != NULL; sr = sr->srq_next)
		{
			...
			sr->srq_dkim = dkim_sign(conf->conf_libphoenixdkim,
			                         dfc->mctx_jobid,
	...
	if (dfc->mctx_srhead != NULL && setidentity)
	{
		...
		if (conf->conf_identityhdr != NULL)
		{
			...
			if (hdr != NULL)
			{
				status = dkim_mail_parse((u_char *) hdr->hdr_val,
				                         &iuser, &idomain);
		...
		if (!idset)
		{
			...
			{
				if (dolog)
					dkimf_log(curconf, LOG_ERR, "identity truncated");
...
mlfi_eom(SMFICTX *ctx)
{
	...
	Header hdr;
	char header[DKIM_MAXHEADER + sizeof(AUTHRESULTSHDR) + 2];
		...
		*/
		...
		if (conf->conf_logresults && conf->conf_dolog)
		{
			...
			if (lstatus == DKIM_STAT_OK)
			{
				...
				for (c = 0; c < nsigs; c++)
				{
					domain = (const char *) dkim_sig_getdomain(sigs[c]);
					selector = (const char *) dkim_sig_getselector(sigs[c]);
		...
		*/
		...
		if (dfc->mctx_addheader)
		{
			...
			*/
			...
			strlcat(header, DELIMITER, sizeof header);
...
main(int argc, char **argv)
{
		  ...
		  case 'V':
			...
			for (c = 0; dkimf_sign[c].str != NULL; c++)
			{
				if (dkimf_sign[c].code != DKIM_SIGN_RSASHA256 ||
	    			    dkim_libfeature(curconf->conf_libphoenixdkim,
				                    DKIM_FEATURE_SHA256))
					printf("\t\t%s\n", dkimf_sign[c].str);
			}
	...
	*/
	...
	if (!dkimf_config_setlib(curconf, &liberr))
	{
		dkimf_log(curconf, LOG_ERR, "can't configure DKIM library: %s",
		       liberr);
	...
	if ((curconf->conf_mode & DKIMF_MODE_VERIFIER) != 0 &&
	    !dkim_libfeature(curconf->conf_libphoenixdkim, DKIM_FEATURE_SHA256))
	{
	...
	if (testpubkeys != NULL)
	{
		...
		(void) dkim_setopt(curconf->conf_libphoenixdkim,
	...
	if (testfile != NULL)
	{
		status = dkimf_testfiles(curconf->conf_libphoenixdkim, testfile,
		                         fixedtime, stricttest, verbose);
		dkim_close(curconf->conf_libphoenixdkim);
		return status;
	}
