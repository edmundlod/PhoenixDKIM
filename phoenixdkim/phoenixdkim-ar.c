...
ares_parse(u_char *hdr, struct authres *ar)
{
	...
	for (c = 0; c < ntoks; c++)
	{
		...
		switch (state)
		{
		  ...
		  case 1:				/* [version] */
			...
			if (tokens[c][0] == ';')
			{
				...
				state = 14;
			         ...
			         isdigit(tokens[c][0]))
			{
				if (strlcpy((char *) ar->ares_version,
				            (char *) tokens[c],
			...
			{
				/*
				 * ADMD-less header (e.g. Office 365): the token
		  ...
		  case 8:
			if (strlcpy((char *) ar->ares_result[n - 1].result_reason,
			            (char *) tokens[c],
		  ...
		  case 9:				/* ptype */
			    ...
			    tokens[c][1] == '\0')
			{
				...
				if (strlcat((char *) ar->ares_result[n - 1].result_value[r],
				            (char *) tokens[c],
		  ...
		  case 11:				/* property */
			if (r < MAXPROPS)
			{
				if (strlcpy((char *) ar->ares_result[n - 1].result_property[r],
				            (char *) tokens[c],
