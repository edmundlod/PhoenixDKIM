...
-- Default (lenient) handling of a KeyTable algorithm mismatch: must sign
--
-- The KeyTable declares ed25519-sha256 for an RSA key, but StrictSignAlgorithm
-- is NOT set.  The mismatch must only warn; signing proceeds with the
-- key-derived algorithm (rsa-sha256).  This pins the default behaviour so the
-- strict option cannot silently become the default.

mt.echo("*** lenient algorithm-mismatch signing test")

-- setup
if TESTSOCKET ~= nil then
	sock = TESTSOCKET
else
	sock = "unix:" .. mt.getcwd() .. "/t-sign-lenient-algmismatch.sock"
end
...
mt.startfilter(binpath .. "/phoenixdkim", "-x",
               "t-sign-lenient-algmismatch.conf", "-p", sock)
...
mt.macro(conn, SMFIC_MAIL, "i", "t-sign-lenient-algmismatch")
...
-- end of message; the filter must accept and sign despite the mismatch
