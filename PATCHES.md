...
| 6 | Debian patch | `insheader.patch` | All calls to `dkimf_insheader(ctx, 1, …)` should be `dkimf_insheader(ctx, 0, …)`. Index 1 inserts *after* the first existing header; index 0 inserts *before all* headers. Affects `Authentication-Results`, `DKIM-Signature`, and `X-OpenDKIM` insertion order. | **Applied**: changed all five `dkimf_insheader(ctx, 1,` → `dkimf_insheader(ctx, 0,` (lines 3266, 3700, 11678, 12507, 12557 at time of patch). | ✓ Fixed |
| 7 | Debian patch | `fix-miltertest-data.patch` | `miltertest/miltertest.c`: the `mt.data()` function asserts `STATE_DATA` (wrong — it transitions *to* DATA from ENVRCPT) and the `mt.header()` function asserts `STATE_ENVRCPT` (wrong — headers come after DATA). The two state checks are swapped. | **Applied**: swapped the two assertions to match SMTP sequencing. Patch by David Buergin (Debian). | ✓ Fixed |
| 8 | Debian patch | `fix-miltertest-eom-check-smtpreply.patch` | `miltertest.c:3693–3694`: when `MT_SMTPREPLY` is checked, `esc` and `text` are passed directly to `snprintf` even when NULL. Undefined behaviour (NULL pointer in `%s` format argument is not guaranteed to produce `"(null)"` on all platforms). | **Applied**: guarded both args — `esc == NULL ? "" : esc` and `text == NULL ? "" : text`. Patch by David Buergin (Debian). | ✓ Fixed |
| 9 | GitHub issue | #222 / #223 | Segfault when `Minimum 100%` is set in config and an inbound message has an empty body. Division by zero: `(100 * canonlen) / bodylen` when `bodylen == 0`. | **Applied**: wrap the entire sigmin block in `if (bodylen)` — empty body is never partial. Fix by Yasuhito FUTATSUKI (upstream PR #223). | ✓ Fixed |
| 10 | GitHub issue | #229 / #230 | `dkimf_config_load()` sign-table consistency walk: `dkimf_db_walk()` error return not checked; second SigningTable field (signer) not marked optional, causing walk to fail on valid single-field entries. | Already fixed in our tree: `_walkret` captures the return, `dbd[1]` marked `DKIMF_DB_DATA_OPTIONAL`, post-loop `_walkret == -1` check present. Applied test config fix (`file:` → `refile:`) so the walk path is exercised. Fix by Yasuhito FUTATSUKI (upstream PR #230). | ✓ Fixed |
| 11 | GitHub issue | #262 | `DKIM_STAT_KEYFAIL` not handled in `mlfi_eoh()` or `mlfi_eom()` inner switch; Lua hook variable shadowed `status` from `dkim_eoh()`; `status=dkimf_libstatus()` stored sfsistat in int; `DKIMF_STATUS_KEYFAIL` missing from A-R check and final status switch. | **Applied**: added `DKIMF_STATUS_KEYFAIL` constant (renumbered 3–9); added status string; fixed Lua hook shadow (`hkstat`); added `DKIM_STAT_KEYFAIL` cases to both switches; fixed `ret`/`status` confusion; added KEYFAIL to A-R check and outer switch; initialised `ret` at declaration. Fix by Yasuhito FUTATSUKI (upstream PR #262). | ✓ Fixed |

### Investigate — lower priority

| Priority | Source | ID / File | Summary | Recommendation | Notes |
|----------|--------|-----------|---------|----------------|-------|
| 12 | GitHub issue | #233 / #234 | When a `DKIM_SIGFLAG_IGNORE`-marked signature is encountered, OpenDKIM records `dkim=fail` in `Authentication-Results` instead of `dkim=policy`. RFC 8601 §2.7.1 distinguishes these results. | **Applied**: cache flags in `sigflag`; check `DKIM_SIGFLAG_IGNORE` first and set `result = "policy"` before the pass/fail chain. Fix by Yasuhito FUTATSUKI (upstream PR #234). | ✓ Fixed |
| 13 | GitHub issue | #244 | `dfc->mctx_domain == NULL` in `dkimf_add_signrequest()` always evaluates false — `mctx_domain` is a char array, not a pointer. Should check `mctx_domain[0] == '\0'`. vbr.c fix not applicable (removed per SCOPE.md). | **Applied**: `dfc->mctx_domain == NULL` → `dfc->mctx_domain[0] == '\0'`. Fix by Jim C. (upstream PR #244). | ✓ Fixed |
| 14 | Debian patch | `nsupdate_output.patch` | `opendkim-genzone -u` (nsupdate mode) outputs bare key data without the required `v=DKIM1; k=rsa;` prefix. (255-byte chunking was already present in our tree.) Also adds `-M` flag to restrict key to email use — skipped, no `mailrestrict`/`subdomains` variable in our genzone. | **Applied**: added `v=DKIM1; k=rsa; p=` to the nsupdate TXT format string. | ✓ Fixed |
| 15 | Debian patch | `suppress-brackets-syslog.patch` | Startup syslog message `opendkim vX.Y.Z starting ()` prints empty parentheses when no arguments are logged. | **Applied**: guard parens with `argstr[0] == '\0'` ternaries — omit `()` entirely when no args. | ✓ Fixed |
