...
mt.startfilter(binpath .. "/phoenixdkim", "-x", "t-verify-ss-header-fields.conf",
               "-p", sock)
...
-- find the Authentication-Results header with dkim=pass and verify all
-- header.X fields are present
n = 0
ar_found = nil
while true do
...
if ar_found == nil then
	error("no Authentication-Results with dkim=pass found")
end

if string.find(ar_found, "header.d=example.com", 1, true) == nil then
	error("missing header.d in Authentication-Results")
end
if string.find(ar_found, "header.s=test", 1, true) == nil then
	error("missing header.s in Authentication-Results")
end
if string.find(ar_found, "header.a=rsa-sha256", 1, true) == nil then
	error("missing header.a in Authentication-Results")
end
if string.find(ar_found, "header.b=", 1, true) == nil then
	error("missing header.b in Authentication-Results")
end
