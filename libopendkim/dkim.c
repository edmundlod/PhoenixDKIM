                 ...
                 void *udata, _Bool syntax, const char *name)
{
	...
	switch (set->set_type)
	{
	  case DKIM_SETTYPE_SIGREPORT:
		...
		pval = dkim_param_get(set, (const u_char *) "rp");
		if (pval != NULL)
		{
			unsigned int tmp = 0;
			char *endptr;

			tmp = (unsigned int) strtoul((const char *) pval,
			                             &endptr, 10);
			if (tmp > 100 || *endptr != '\0')
			{
				dkim_error(dkim,
	  ...
	  case DKIM_SETTYPE_SIGNATURE:
		...
		pval = dkim_param_get(set, (const u_char *) "d");
		for (; *pval != '\0'; pval++)
		{
			if (!(isalpha(*pval) ||
			      isdigit(*pval) ||
		...
		pval = dkim_param_get(set, (const u_char *) "t");
		if (pval != NULL)
		{
			uint64_t tmp = 0;
			char *end;
			...
			{
				tmp = strtoull((const char *) pval, &end, 10);
			}
		...
		pval = dkim_param_get(set, (const u_char *) "x");
		if (pval != NULL)
		{
			uint64_t tmp = 0;
			char *end;
...
dkim_privkey_load(DKIM *dkim)
{
	...
	keybio = BIO_new_mem_buf(dkim->dkim_key, dkim->dkim_keylen);
	if (keybio == NULL)
	{
	...
	if (dkim->dkim_signalg == DKIM_SIGN_ED25519SHA256)
	{
		...
		ed->ed_pkey = pkey;
		ed->ed_siglen = EVP_PKEY_get_size(pkey);
		ed->ed_sig = DKIM_MALLOC(dkim, ed->ed_siglen);
		if (ed->ed_sig == NULL)
		{
			dkim_error(dkim, "unable to allocate %d byte(s)",
			           ed->ed_siglen);
	...
	else
	{
		...
		rsa->rsa_pkey = pkey;
		rsa->rsa_keydata = keybio;
		rsa->rsa_keysize = EVP_PKEY_get_bits(pkey);
		rsa->rsa_rsaout = DKIM_MALLOC(dkim, EVP_PKEY_get_size(pkey));
...
static _Bool
dkim_key_hashok(DKIM_SIGINFO *sig, const u_char *hashlist)
{
	int hashalg;
	const u_char *x, *y;
	u_char tmp[BUFRSZ + 1];
	...
	for (;;)
	{
		if (*y == ':' || *y == '\0')
		{
			if (x != NULL)
			{
				strlcpy((char *) tmp, (const char *) x, sizeof tmp);
				tmp[y - x] = '\0';
...
static _Bool
dkim_key_hashesok(DKIM_LIB *lib, const u_char *hashlist)
{
	const u_char *x, *y;
	u_char tmp[BUFRSZ + 1];
...
dkim_sig_hdrlistok(DKIM *dkim, const u_char *hdrlist)
{
	...
	for (d = 0; ; d++)
	{
		...
		for (c = 0; c < nh; c++)
		{
			if (strcasecmp((const char *) required_signhdrs[d],
			               (char *) ptrs[c]) == 0)
			{
				found = TRUE;
				break;
...
dkim_sig_domainok(DKIM *dkim, DKIM_SET *set)
{
	...
	if (strcasecmp(at + 1, (const char *) d) == 0)
		return TRUE;

	for (dot = strchr(at, '.'); dot != NULL; dot = strchr(dot + 1, '.'))
	{
		if (strcasecmp(dot + 1, (const char *) d) == 0)
		{
...
dkim_sig_expired(DKIM_SET *set, uint64_t drift)
{
	...
	if (sizeof(uint64_t) == sizeof(unsigned long long))
		expire = strtoull((const char *) val, NULL, 10);
	else if (sizeof(uint64_t) == sizeof(unsigned long))
		expire = strtoul((const char *) val, NULL, 10);
	else
		expire = (unsigned int) strtoul((const char *) val, NULL, 10);
...
dkim_sig_timestampsok(DKIM_SET *set)
{
	...
	if (val == NULL)
		return TRUE;
	if (sizeof(uint64_t) == sizeof(unsigned long long))
		signtime = strtoull((const char *) val, NULL, 10);
	else if (sizeof(uint64_t) == sizeof(unsigned long))
		signtime = strtoul((const char *) val, NULL, 10);
	else
		signtime = (unsigned int) strtoul((const char *) val, NULL, 10);
...
dkim_sig_versionok(DKIM *dkim, DKIM_SET *set)
{
	...
	v = (const char *) dkim_param_get(set, (const u_char *) "v");
	     ...
	     set = dkim_set_next(set, DKIM_SETTYPE_SIGNATURE), c++)
	{
		...
		*/
		...
		else
		{
			...
			strlcpy(value, (const char *) param, sizeof value);
		...
		if (param != NULL)
		{
			...
			*/
			...
			errno = 0;
			ul = strtoul((const char *) param, &q, 10);
		...
		if (param != NULL)
		{
			...
			strlcpy((char *) qtype, (const char *) param, sizeof qtype);
		...
		else
		{
			if (sizeof(uint64_t) == sizeof(unsigned long long))
			{
				dkim->dkim_siglist[c]->sig_timestamp = strtoull((const char *) param,
				                                                NULL,
				                                                10);
			}
			else if (sizeof(uint64_t) == sizeof(unsigned long))
			{
				dkim->dkim_siglist[c]->sig_timestamp = strtoul((const char *) param,
				                                               NULL,
				                                               10);
			}
			else
			{
				dkim->dkim_siglist[c]->sig_timestamp = (unsigned int) strtoul((const char *) param,
				                                                              NULL,
				                                                              10);
			}
		}
		...
		b64siglen = strlen((const char *) param);
               ...
               const char *delim)
{
	...
	*/
	...
	for (hdr = dkim->dkim_hhead; hdr != NULL; hdr = hdr->hdr_next)
	{
		...
		{
			dkim_dstring_cat1(dstr, ';');
			dkim_dstring_catn(dstr, (const u_char *) delim, delimlen);
...
dkim_get_key(DKIM *dkim, DKIM_SIGINFO *sig, _Bool test)
{
	...
	if (p != NULL && strcmp((const char *) p, DKIM_VERSION_KEY) != 0)
	{
	...
	else if (dkim_name_to_code(keytypes, (const char *) p) == -1)
	{
	...
	if (p != NULL)
	{
		...
		strlcpy(tmp, (const char *) p, sizeof tmp);
...
dkim_eom_sign(DKIM *dkim)
{
	...
	switch (sig->sig_signalg)
	{
	  case DKIM_SIGN_RSASHA1:
		dkim_error(dkim,
		           "RSA-SHA1 signing is not supported");
		return DKIM_STAT_INVALID;
	  ...
	  case DKIM_SIGN_RSASHA256:
	  {
		...
		pctx = EVP_PKEY_CTX_new(rsa->rsa_pkey, NULL);
		if (pctx == NULL)
		{
			dkim_load_ssl_errors(dkim, 0);
		    ...
		    EVP_PKEY_CTX_set_signature_md(pctx, EVP_sha256()) != 1)
		{
			dkim_load_ssl_errors(dkim, 0);
			dkim_error(dkim,
			           "EVP_PKEY signing context setup failed");
		...
		outlen = EVP_PKEY_get_size(rsa->rsa_pkey);

		status = EVP_PKEY_sign(pctx, rsa->rsa_rsaout, &outlen,
		                       digest, diglen);
	  ...
	  case DKIM_SIGN_ED25519SHA256:
	  {
		...
		ed = (struct dkim_ed25519 *) sig->sig_signature;
		...
		outlen = ed->ed_siglen;
		status = EVP_DigestSign(mdctx, ed->ed_sig, &outlen,
		                        digest, diglen);
		if (status != 1 || outlen == 0)
		{
			dkim_load_ssl_errors(dkim, 0);
			dkim_error(dkim,
			           "Ed25519 signature generation failed (status %d, length %zu)",
			           status, outlen);
			EVP_MD_CTX_free(mdctx);
			return DKIM_STAT_INTERNAL;
		}
...
dkim_close(DKIM_LIB *lib)
{
	...
	if (lib->dkiml_oversignhdrs != NULL)
		dkim_clobber_array((const char *const *) lib->dkiml_oversignhdrs);

	if (lib->dkiml_requiredhdrs != dkim_required_signhdrs)
		dkim_clobber_array((const char *const *) lib->dkiml_requiredhdrs);

	if (lib->dkiml_mbs != NULL)
		dkim_clobber_array((const char *const *) lib->dkiml_mbs);
	...
	switch (opt)
	{
	  ...
	  case DKIM_OPTS_SIGNHDRS:
		...
		else
		{
			...
			buflen = 2				/* "^(" */
			       + dkim_hdrlist_bufsize(required_signhdrs)
			       + dkim_hdrlist_bufsize((const u_char *const *) ptr)
			       + 2				/* ")$" */
			       + 1;				/* '\0' */
			...
			if (!dkim_hdrlist((u_char *) buf, buflen,
			    ...
			    !dkim_hdrlist((u_char *) buf, buflen,
			                  (const u_char *const *) ptr, FALSE) ||
...
dkim_free(DKIM *dkim)
{
	...
	if (dkim->dkim_siglist != NULL)
	{
		...
		for (c = 0; c < dkim->dkim_sigcount; c++)
		{
			...
			else if (dkim->dkim_siglist[c]->sig_keytype == DKIM_KEYTYPE_ED25519)
			{
				struct dkim_ed25519 *ed;

				ed = dkim->dkim_siglist[c]->sig_signature;
				if (ed != NULL)
				{
					EVP_CLOBBER(ed->ed_pkey);
					if (ed->ed_sig != dkim->dkim_siglist[c]->sig_sig)
						CLOBBER(ed->ed_sig);
				}
			}
...
dkim_sig_process(DKIM *dkim, DKIM_SIGINFO *sig)
{
	...
	*/
	...
	if ((sig->sig_flags & DKIM_SIGFLAG_PROCESSED) == 0)
	{
		...
		if (sig->sig_signalg == DKIM_SIGN_ED25519SHA256)
		{
			...
			if (ed->ed_pkey == NULL)
			{
				...
				dkim_error(dkim,
				           "s=%s d=%s: EVP_PKEY_new_raw_public_key() failed",
			...
			ed->ed_sig = sig->sig_sig;
			ed->ed_siglen = sig->sig_siglen;
			ed->ed_keydata = sig->sig_key;
			ed->ed_keylen = sig->sig_keylen;

			sig->sig_keybits = 8 * sig->sig_keylen;
			sig->sig_keytype = DKIM_KEYTYPE_ED25519;
			...
			rsastat = EVP_DigestVerify(mdctx, ed->ed_sig,
			                           ed->ed_siglen,
			                           digest, diglen);
		...
		else
		{
...

			{
				...
				if (pctx == NULL)
				{
					dkim_sig_load_ssl_errors(dkim, sig, 0);
					dkim_error(dkim,
					           "s=%s d=%s: EVP_PKEY_CTX_new() failed",
					           dkim_sig_getselector(sig),
					           dkim_sig_getdomain(sig));
				    ...
				    EVP_PKEY_CTX_set_signature_md(pctx, md) != 1)
				{
					dkim_sig_load_ssl_errors(dkim, sig, 0);
					dkim_error(dkim,
					           "s=%s d=%s: EVP_PKEY verify context setup failed",
					           dkim_sig_getselector(sig),
					           dkim_sig_getdomain(sig));
				...
				rsastat = EVP_PKEY_verify(pctx, rsa->rsa_rsain,
				                          rsa->rsa_rsainlen,
				                          digest, diglen);
	    ...
	    (sig->sig_flags & DKIM_SIGFLAG_PASSED) != 0)
	{
		...
		if (strcmp((const char *) bhash, (char *) b64buf) == 0)
		{
			sig->sig_bh = DKIM_SIGBH_MATCH;
		}
		else
	...
	*/
	...
	if ((sig->sig_flags & DKIM_SIGFLAG_NOSUBDOMAIN) != 0)
	{
		...
		d = (const char *) dkim_param_get(sig->sig_taglist,
		                                  (const u_char *) "d");
		i = (const char *) dkim_param_get(sig->sig_taglist,
		                                  (const u_char *) "i");
...
#define	DELIMITER	"\001"
	...
	else
	{
		     ...
		     pv = strtok_r(NULL, DELIMITER, &ctx))
		{
			...
			else if (sig->sig_keytype == DKIM_KEYTYPE_ED25519)
			{
				u_int siglen;

				siglen = BASE64SIZE(64);
				if (strcmp(which, "b") == 0 &&
				    len + whichlen + siglen + 1 >= dkim->dkim_margin)
					forcewrap = TRUE;
			}
                       ...
                       u_int *pct)
{
	...
	if (opts != NULL)
	{
		...
		if (p != NULL)
			strlcpy((char *) opts, (const char *) p, optslen);
	}
...
dkim_sig_getidentity(DKIM *dkim, DKIM_SIGINFO *sig, u_char *val, size_t vallen)
{
	...
	param = (const char *) dkim_param_get(set, (const u_char *) "i");
	if (param == NULL)
	{
		param = (const char *) dkim_param_get(set, (const u_char *) "d");
		if (param == NULL)
			return DKIM_STAT_INTERNAL;
...
**
**  Return value:
**  	Pointer to the algorithm name associated with the DKIM_SIGINFO.
*/
...
dkim_get_sigsubstring(DKIM *dkim, DKIM_SIGINFO *sig, char *buf, size_t *buflen)
{
	...
	if (dkim->dkim_minsiglen == 0)
	{
		...
		*/
		...
		{
			b1 = (const char *) dkim_param_get(dkim->dkim_siglist[c]->sig_taglist,
			                                   (const u_char *) "b");
			if (b1 == NULL)
				continue;
			...
			{
				b2 = (const char *) dkim_param_get(dkim->dkim_siglist[d]->sig_taglist,
				                                   (const u_char *) "b");
				if (b2 == NULL)
					continue;
	...
	b1 = (const char *) dkim_param_get(sig->sig_taglist, (const u_char *) "b");
	if (b1 == NULL)
		return DKIM_STAT_SYNTAX;
...
dkim_signhdrs(DKIM *dkim, const char **hdrlist)
{
	...
	if (hdrlist != NULL)
	{
		...
		if (buf == NULL)
		{
			dkim_error(dkim, "could not allocate %d bytes", buflen);
			return DKIM_STAT_INTERNAL;
		}
